...
大模型规划分析 + 小模型执行 + 强模型复核 的自动化编程 CLI 工具。
...
### ⚡ 一键安装（推荐）
```bash
curl -sSL https://raw.githubusercontent.com/LiPingStruggle/lingshu/main/install.sh | bash
```
...
# 提交单个任务（支持文件/目录自动搜索）
lǐngshu run "分析空指针bug" --code G:\aosp\kernel\ --depth deep

# 智能调度模式（多模型并发）
lǐngshu smart "分析并修复登录模块的认证逻辑缺陷"
...
# 查看任务状态
lǐngshu list
lǐngshu status <task_id>

# 从断点恢复
lǐngshu resume <task_id>
...
# 插件管理
lǐngshu plugin list
lǐngshu plugin run --phase evidence_collection

# 成本分析
lǐngshu cost summary

# 教会 AI 规则
lǐngshu teach "coding_style = 函数式优先"
```
...
```bash
lǐngshu plugin list              # 列出插件
lǐngshu plugin run --phase evidence_collection  # 执行阶段
```
...
| 命令 | 说明 |
|------|------|
| `lingshu run <desc> --code <path>` | 执行单个任务，自动搜索源码 |
| `lingshu batch` | 批量执行任务清单 |
| `lingshu resume <id>` | 从检查点恢复 |
| `lingshu list [status]` | 列出任务 |
| `lingshu status <id>` | 查看任务详情 |
| `lingshu server` | 启动 RPC 服务 |
| `lingshu engine <hours>` | 启动持续运行引擎 |
| `lingshu smart <desc>` | 智能调度模式 |
| `lingshu trust [action]` | 反向验证器信任管理 |
| `lingshu teach <rule>` | 教会 AI 规则 |
| `lingshu feedback <action>` | 反馈学习器管理 |
| `lingshu map` | 生成仓库地图 |
| `lingshu cost summary` | 成本汇总 |
| `lingshu plugin list / run` | 插件管理 |
